...
var natOne = bigmod.NewNat().SetBig(big.NewInt(1))
var natTwo = bigmod.NewNat().SetBig(big.NewInt(2))
...
func ctCondCopy(v choice, x, y []uint) {
	if len(x) != len(y) {
...
func cmpGeq(x []uint, y []uint) choice {
	var c uint
...
func CondSwap(cond choice, x *bigmod.Nat, y *bigmod.Nat, m *bigmod.Modulus) {
...
func ctIfElse(v choice, x, y uint) uint {
...
func shiftAddInGeneric(z, scratch []uint, x uint, m *bigmod.Nat) uint {
	...
	if size == 1 {
		...
		m0,err := m.GetWord(0)
		if err!=nil {
			panic("invalid index in GetWord")
		}
		q, r := div(z[0], x, m0)
		z[0] = r
		return q
	...
	for i := size - 1; i > 1; i-- {
		...
		mi,err := m.GetWord(uint(i))
		if err != nil {
			panic("invalid index in GetWord")
		}
		b1 = ctIfElse(done, b1, mi)
		mi1,err := m.GetWord(uint(i-1))
		if err != nil {
			panic("invalid index in GetWord")
		}
		b0 = ctIfElse(done, b0, mi1)
		done = 1 ^ ctEq(b1, 0)
	}
	...
	return shiftAddInCommon(z, scratch, m.Limbs[0:m.Length()], hi, a2, a1, b1)
}
...
func alias(x, y *bigmod.Nat) bool {
	return cap(x.Limbs) > 0 && cap(y.Limbs) > 0 && &x.Limbs[0:cap(x.Limbs)][cap(x.Limbs)-1] == &y.Limbs[0:cap(y.Limbs)][cap(y.Limbs)-1]
}

// z = x << s
func shl(z* bigmod.Nat, x *bigmod.Nat, s uint) *bigmod.Nat {
	if s == 0 {
		if same(z, x) {
			return z
		}
	...
	n := m + int(s/_W)
	z.Reset(n+1)
	z.SetWord(uint(n), shlVU(z.Limbs[n-m:n], x, s%_W))
	z.ClearWords(0, uint(n-m))
...
func shr(z *bigmod.Nat, x *bigmod.Nat, s uint) *bigmod.Nat {
	...
	z = x.Reset(n)
	shrVU(z, x.Limbs[m-n:], s%_W)
	return norm(z)
}

func norm(z *bigmod.Nat) *bigmod.Nat {
	i := z.Length()
...
func getrandom32() uint32 {
	num32 := make([]byte, 4)
	if _, err := rand.Read(num32); err != nil {
		panic("crypto/rsa: RNG failure")
	}
	y := binary.BigEndian.Uint32(num32)
	return y
...
func random(z *bigmod.Nat, limit *bigmod.Nat, n int) *bigmod.Nat {
	if alias(z, limit) {
	...
	for {
		...
		case 32:
			for i := range z.Limbs {
				z.Limbs[i] = uint(getrandom32())
			}
		case 64:
			for i := range z.Limbs {
				z.Limbs[i] = uint(getrandom32()) | uint(getrandom32())<<32
			}
		default:
			panic("unknown word size")
		}
		z.Limbs[limit.Length()-1] &= mask
		if cmp(z, limit) < 0 {
			break
		}
	}
...
func probablyPrimeMillerRabin(n *bigmod.Nat, reps int, force2 bool) bool {
...
NextRandom:
	for i := 0; i < reps; i++ {
		if i == reps-1 && force2 {
			...
			x = random( x, nm3, nm3Len)
			x = x.add(x, natTwo)
		}
		y = y.Exp(x, q.Bytes(), nmod)
		if cmp(y, natOne) == 0 || cmp(y, nm1) == 0 {
			continue
		}
		for j := uint(1); j < k; j++ {
...
func (z *bigmod.Nat) invert(announced int, x *bigmod.Nat, m *bigmod.Nat, m0inv uint) bool {
	...
	copy(b, m)
	scratch := z.Limbs[4*(size+1):z.Length()]
...
func rsa_probablyPrime(x *bigmod.Nat, n int) bool {
	...
	return rsa_probablyPrimeMillerRabin(x, n+1, true) &&rsa_probablyPrimeLucas(x)
}
...
func (z *bigmod.Nat) sqrt(x *bigmod.Nat) *bigmod.Nat {
	...
	z1.set(z)
	z1 = z1.setUint64(1)
	z1 = shl(z1, z1, uint(bitLen(x)+1)/2) // must be ≥ √x
	for n := 0; ; n++ {
...
func  cmp(x *bigmod.Nat, y *bigmod.Nat) (r int) {
        m := x.Length()
        n := y.Length()
        if m != n || m == 0 {
                switch {
...
func (n *bigmod.Nat) probablyPrimeLucas() bool {
	...
	if cmp(vk, natTwo) == 0 || cmp(vk, nm2) == 0 {
		// Check U(s) ≡ 0.
	...
	for {
		...
		*/
		if rsa_coprime(diff, ENat, Nmod) {
			//if ret.Cmp(bigOne) == 0 {
			ret := rsa_probablyPrime(p, rounds)
...
genq:
	...
	for {
		...
		if rsa_coprime(diff, ENat, Nmod) {
			ret := rsa_probablyPrime(q, rounds)
			//	fmt.Printf("check for probably prime returned %t", ret)
			if ret == true {
